...
export default function ResultsReport({ tableName, onClose }: ResultsReportProps) {
  ...
  const handleSelectAll = () => {
    const allIds = new Set(filteredPosts.map((post, index) => index.toString()))
  ...
  const filterByThumbnail = (type: string) => {
    ...
    // 실제 필터링은 서버 사이드에서 하거나 전체 데이터를 가져와서 해야 함
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page)
    setSelectedPosts(new Set()) // 페이지 변경시 선택 초기화
  }
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <div className="text-sm text-gray-600">
                  Total {totalCount} | Page {currentPage} of {pagination?.totalPages || 1} | Filtered {filteredPosts.length} | Selected {selectedPosts.size}
                </div>
              </div>
          ...
          <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            {groupStats.length > 0 && (
              <div className="flex gap-2 mb-6 flex-wrap">
                <button className="group-filter-btn active">
                  ALL ({posts.length})
                </button>
